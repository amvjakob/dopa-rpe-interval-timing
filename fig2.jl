...
function plot_dict(dict::Dict{String, Vector{Float64}}; 
    win=1, title="Plot", xlabel="Lick-adjusted time", ylabel="Value", show_grid_label=true)
    
    pyplot()
    nbins = length(dict[collect(keys(dict))[1]])
    binlen = 17 / nbins
    ...
    for (key, value) in dict
        plot!(x, smooth(value, :rect, win), label=key)
    end

    title!(title)
    vline!([0], color="black", opacity=0.2, linestyle=:dash, label=show_grid_label ? "first lick" : "")
    hline!([0], color="black", opacity=0.2, linestyle=:dash, label=show_grid_label ? "y = 0" : "")
    xlabel!(xlabel)
    ylabel!(ylabel)
...
function plot_contribution(dict::Dict; 
    win=1, title="Plot", xlabel="Lick-adjusted time", ylabel="Value", show_grid_label=true)
    
    sessions = collect(keys(dict))
    analyses = collect(keys(dict[sessions[1]]))
    
    nbins = length(dict[sessions[1]][analyses[1]])
    binlen = 17 / nbins
    ...
    for analysis in analyses
        values = [val[analysis] for (session, val) in dict]
        
        y      = mean(values)
        stderr = std(values) ./ sqrt(length(values))
        
        plot!(x, smooth(y, :rect, win), ribbon=stderr, label=analysis)
    end
